          ...
          >
            ...
            {isAccountLoading ? (
              ...
              <>
                {/* Account Name */}
                <div>
                  <label className={LABEL_CLASS} htmlFor="accountName">
                    Account / Store Name
                  </label>
                  <input
                    id="accountName"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() => setAccountForm((prev) => ({ ...prev, status: 'ACTIVE' }))}
                      className={`flex items-center justify-center gap-2 p-3 rounded-lg border text-sm font-semibold transition-colors ${
                        accountForm.status === 'ACTIVE'
                          ? 'bg-emerald-500/15 border-emerald-500 text-emerald-300'
                          : 'bg-[var(--color-bg-primary)] border-[var(--color-border)] text-neutral-400 hover:text-white'
                      }`}
                    >
                      <CheckCircle2 size={16} />
                      ACTIVE
                    ...
                    <button
                      type="button"
                      onClick={() => setAccountForm((prev) => ({ ...prev, status: 'SUSPENDED' }))}
                      className={`flex items-center justify-center gap-2 p-3 rounded-lg border text-sm font-semibold transition-colors ${
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className={LABEL_CLASS + " mb-0"} htmlFor="dueDate">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      <button
                        ...
                        className="px-2 py-0.5 rounded bg-neutral-800 text-neutral-300 hover:text-white border border-neutral-700 transition-colors"
                      >
                        +30 Days
                      </button>
                      <button
                        type="button"
                        onClick={() => setAccountForm((prev) => ({ ...prev, dueDate: '' }))}
                        className="px-2 py-0.5 rounded bg-neutral-800 text-neutral-400 hover:text-red-400 border border-neutral-700 transition-colors"
                      >
                        Clear
                      </button>
                    </div>
                  ...
                  <input
                    ...
                    onChange={(e) => setAccountForm((prev) => ({ ...prev, dueDate: e.target.value }))}
                    className={FIELD_CLASS}
                  />
                  <p className="text-[11px] text-[var(--color-text-muted)] mt-1">
                    When remaining days are less than 5, a warning banner is shown on the dashboard and main pages. If the date passes without payment, the account is automatically suspended.
                  </p>
                </div>
                ...
                {accountForm.status === 'SUSPENDED' && (
                  <div>
                    <label className={LABEL_CLASS} htmlFor="suspendReason">
                      Suspension Reason
                    </label>
                    <input
                      id="suspendReason"
                      type="text"
                      placeholder="e.g. Monthly subscription payment not received"
                      value={accountForm.suspendReason}
                      onChange={(e) => setAccountForm((prev) => ({ ...prev, suspendReason: e.target.value }))}
                      className={FIELD_CLASS}
                    />
                  </div>
                )}
                ...
                {accountMutation.isError && (
                  <div className="flex items-center gap-2 text-sm text-red-400 bg-red-950/30 border border-red-800/30 rounded-lg px-4 py-3">
                    <AlertCircle size={16} />
                    {(accountMutation.error as { response?: { data?: { message?: string } } })?.response?.data?.message ||
